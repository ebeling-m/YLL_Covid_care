...
# Load datafiles
dat <- read_delim("data_inter/aggreagted_data_transitions.txt", delim = " ")

# Restrict to ages 70 to 104 
# Calculate start and end year 
# Select only data prior 2020
dat_exl <- 
  dat %>%
  filter(AgeGr >= 70 &  AgeGr < 105 & exclude == 0) %>% 
  mutate(sY = Coh+AgeGr, 
         eY = Coh+AgeGr+1.25) %>%
...
# Collapse data for smoothing
# # only selected municipalities 
# dat1_pop_exl <- 
#   dat_exl %>%
#   distinct(sex, AgeGr, Start, YearGr, .keep_all = TRUE) %>% 
...
#               End = as.numeric(1:4), sex = 0:1, YearGr = 15:19) %>% 
...
# dat1_exl <- 
#   ref.dat %>% 
#   left_join(dat1_cases_exl) %>%
#   left_join(dat1_pop_exl) %>% 
...
# # test <- 
# #   dat1 %>%
# #   filter(YearGr %in% c(19) & sex == 1 & Start == 1& AgeGr == 100)
# x <- dat1 %>% filter(sex == 0 & Start == 2 & End == 2 & YearGr == 19)
# range(x$AgeGr)
...
# fit.dat_exl %<>% mutate(fitp_med = pCase/Tpop,
#                     fitp_low = low/Tpop,
#                     fitp_up = up/Tpop)
...
# setwd(wd.dir)
# save(fitfinal_exl, file = "aggregtedData_fittedProb_exl.RData")
